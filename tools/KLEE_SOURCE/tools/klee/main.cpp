...
#ifdef MONICA
	//printf("argc = %d; argv[1] is \" %s \"",argc, argv[1]
#endif
    sys::PrintStackTraceOnErrorSignal();

	//****************************  Forking Watchdog Process*************************************//
    if (Watchdog) {
        if (MaxTime==0) {
...
#ifdef MONICA
branch_cnt =0;
//printf("begin of Monica's KLEE\n");
#endif

//**********End Adding
	//****************************  No Watchdog Part: Doing real business*************************************//

    sys::SetInterruptFunction(interrupt_handle);

    // Load the bytecode...
	//****************************  Load LLVM Bytecode & Set Library*************************************//
    std::string ErrorMsg;
#if LLVM_VERSION_CODE < LLVM_VERSION(2, 7)
    ModuleProvider *MP = 0;
    ...
    IOpts.MakeConcreteSymbolic = MakeConcreteSymbolic;
		...
		//***creat Klee Handler： "handler"
    KleeHandler *handler = new KleeHandler(pArgc, pArgv);
	
		//***creat Interpreter："theInterpreter"
    Interpreter *interpreter =
        theInterpreter = Interpreter::create(IOpts, handler);
    handler->setInterpreter(interpreter);
		
		//***begin writing Info File
    ...
    if (ReplayPathFile != "") {
		...
		//***record start time into Info File
    char buf[256];
    time_t t[2];
    t[0] = time(NULL);
    ...
    if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) {
        ...
        assert(SeedOutDir.empty());
			...
			//***Build Ktests from ReplayOutFile , set working directory
...
//**************************************Run Each Test**********************************************************//
        ...
        while (!kTests.empty()) {
			...
			//*** when ReplayOutDir.empty() && ReplayOutFile.empty()
			//*** build seeds for Ktest
    ...
    stats << "\n";
#ifdef MONICA
    stats << "BLEE: done: total instructions = "
          << instructions << "\n";
    stats << "BLEE: done: completed paths = "
          << handler->getNumPathsExplored() << "\n";
    stats << "BLEE: done: generated tests = "
