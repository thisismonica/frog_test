...
/* Define signature of MW initialization callback
   Arguments:   NU_MEMORY_POOL*     memory pool
                ...
                VOID*               middleware callback parameter
                VOID*               unused */
typedef VOID            (*NMI_INIT_CB_PTR)      (NU_MEMORY_POOL*, NU_MEMORY_POOL*, VOID**, VOID*);

/* Define signature of MW cleanup callback
   Arguments:   VOID*               middleware callback parameter
                VOID*               unused */
typedef VOID            (*NMI_CLEANUP_CB_PTR)   (VOID**, VOID*);

/* Define MW registration data structure filled in by MW registration function */
typedef struct
{
    /* Middleware fills in this data */
    const CHAR*         mw_id_ptr;              /* MW ID (static product-specific string) */
    const CHAR*         mw_ver_ptr;             /* MW version (static product-specific string) */
    VOID*               mw_cb_param;            /* (optional) MW callback parameter passed to callbacks */
...
extern  NMI_INIT_STATUS NMIE_Init_Status_Get    (const CHAR* mw_id_ptr);
extern  NMI_INIT_STATUS NMIE_Init_Status_Set    (const CHAR* mw_id_ptr, NMI_INIT_STATUS mw_status);
extern  NMI_INIT_STATUS NMIE_Wait_For_Init      (const CHAR* mw_id_ptr, UNSIGNED timeout_ticks);
extern  NMI_INIT_STATUS NMIE_Cleanup_Invoke     (const CHAR* mw_id_ptr);

/* Define externally available functions */

extern  VOID            NMI_Initialize          (NU_MEMORY_POOL *mem_pool, NU_MEMORY_POOL *uncached_mem_pool);
extern  NMI_INIT_STATUS NMI_Init_Status_Get     (const CHAR* mw_id_ptr);
extern  NMI_INIT_STATUS NMI_Init_Status_Set     (const CHAR* mw_id_ptr, NMI_INIT_STATUS mw_status);
extern  NMI_INIT_STATUS NMI_Wait_For_Init       (const CHAR* mw_id_ptr, UNSIGNED timeout_ticks);
extern  NMI_INIT_STATUS NMI_Cleanup_Invoke      (const CHAR* mw_id_ptr);
