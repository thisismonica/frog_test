...
typedef struct
{
    UINT32              obj_id;         /* Object ID for the given object                   */
...
typedef struct
{
    UINT32              avail_objects;  /* Number of objects available                      */
    UINT32              data_objects;   /* Number of objects being used as data objects     */
    UINT32              seg_objects;    /* Number of objects being used as segment objects  */
    UINT32              buf_objects;    /* Number of objects being used as buffer objects   */
...
typedef struct
{
    ...
    UINT32          avail_head_id;  /* Object ID of first available object                      */
    UINT32          avail_tail_id;  /* Object ID of last available object                       */
    UINT32          count_bits;     /* Number of bits used as count bits for the unique
                                       32-bit ID for buffers                                    */
    UINT32          count_bit_mask; /* Value used to mask out all but count bits within a
                                       32-bit ID for a buffer                                   */
    UINT32          total_objects;  /* Total number of objects in the system                    */
...
typedef struct
{
    ...
    UINT32          type;           /* Set to ZC_Available_Type                                 */
    UINT32          unused;         /* Not used for ZC Available object                         */
    UINT32          next_obj_id;    /* ID of next available object                              */
...
typedef struct
{
    ...
    UINT32          type;           /* ZC_DATA_TYPE                                             */
    UINT32          unused;         /* Not used for ZC Data object                              */
    UINT32          data_refs;      /* Number of ZC Segments that reference this data           */
    UINT32          mem_ptr;        /* Pointer to the data buffer that this object references   */
    UINT32          length;         /* Length of the memory referenced by this object           */
    UINT32          cb_func_ptr;    /* Pointer to the application supplied call-back function   */
    UINT32          cb_param;       /* Application supplied parameter for the call-back function*/
...
typedef struct
{
    ...
    UINT32          type;           /* ZC_SEGMENT_TYPE                                          */
    UINT32          unused;         /* Not used for ZC Segment object                           */
    UINT32          parent_id;      /* ID of the ZC Buffer that the Segment is part of          */
    UINT32          prev_seg_id;    /* ID of Previous Segment in the ZC Buffer (may be NULL)    */
    UINT32          next_seg_id;    /* ID of Next Segment in the ZC Buffer (may be NULL)        */
    UINT32          data_obj_id;    /* ID to the ONE ZC DATA object referenced by this Segment  */
    UINT32          mem_offset;     /* Positive offset from the start of the memory buffer
                                       pointed to by this segment to the memory referenced.     */
    UINT32          length;         /* Length of the memory (from the mem_offset) referenced by
                                       this Segment.                                            */
...
typedef struct
{
    ...
    UINT32          type;           /* ZC_BUFFER_TYPE                                           */
    UINT32          id;             /* Holds ZC_Object_Id & ZC_Object_Count_Bits                */
    UINT32          head_seg_id;    /* ID of First Segment in the ZC Buffer (may be NULL)       */
    UINT32          tail_seg_id;    /* ID of Last Segment in the ZC Buffer (may be NULL)        */
    UINT32          length;         /* Total number of data bytes contained the ZC Buffer       */
...
typedef             UINT32                      ZC_OBJ_ID;
typedef             UINT32                      ZC_OBJ_TYPE;
...
#define ZC_SEG_OPTIMAL_ENSURE(buf_id, seg_id, offset_ptr)                                   \
            (((*(offset_ptr) >= 0) && ((seg_id) != NU_NULL) &&                              \
              (*(offset_ptr) < (INT)((ZC_OBJ_PTR_GET(seg_id)) -> seg.length))) ? (seg_id) : \
...
#define ZC_BUF_END_CHECK(buf_obj, seg_id, offset)                                       \
            ((((buf_obj) -> buf.tail_seg_id) == (seg_id)) &&                            \
             ((offset) == (INT)((ZC_OBJ_PTR_GET(seg_id)) -> seg.length)))
...
#define ZC_4BYTE_ALIGN_CHECK(mem_addr)                                                  \
            (((UINT32)(mem_addr) & 3UL) == 0)

/* Define macro to align memory address on 4-byte boundary:
   ADD the size of a UINT32 to the memory address then AND the result to clear the
   lower 2 bits */
#define ZC_4BYTE_ALIGN_GET(mem_addr)                                                    \
            (VOID *)(((UINT32)(mem_addr) + sizeof(UINT32)) & 0xFFFFFFFCUL)
...
STATUS      ZC_INITIALIZE(NU_MEMORY_POOL *, INT);
ZC_BUF_ID   ZC_BUF_CREATE(VOID);
STATUS      ZC_BUF_DELETE(ZC_BUF_ID);
ZC_SEG_ID   ZC_BUF_INSERT(ZC_BUF_ID, ZC_SEG_ID, INT, ZC_BUF_ID);
ZC_BUF_ID   ZC_BUF_SPLIT(ZC_BUF_ID, ZC_SEG_ID, INT);
ZC_BUF_ID   ZC_BUF_DUPLICATE(ZC_BUF_ID, ZC_SEG_ID, INT, INT);
ZC_SEG_ID   ZC_DATA_INSERT(ZC_BUF_ID, ZC_SEG_ID, INT, CHAR *, INT, VOID (*)(CHAR *, INT), INT);
ZC_SEG_ID   ZC_DATA_COPY_INSERT(ZC_BUF_ID, ZC_SEG_ID, INT, CHAR *, INT);
ZC_SEG_ID   ZC_DATA_REMOVE(ZC_BUF_ID, ZC_SEG_ID, INT, INT);

/* Define function prototypes for Zero Copy PASSIVE functions */
INT         ZC_BUF_LEN_GET(ZC_BUF_ID);
INT         ZC_DATA_COPY_GET(ZC_BUF_ID, ZC_SEG_ID, INT, CHAR *, INT);
ZC_SEG_ID   ZC_SEG_OPTIMAL_GET(ZC_BUF_ID, ZC_SEG_ID, INT *);
ZC_SEG_ID   ZC_SEG_NEXT_GET(ZC_BUF_ID, ZC_SEG_ID);
ZC_SEG_ID   ZC_SEG_PREV_GET(ZC_BUF_ID, ZC_SEG_ID);
CHAR *      ZC_SEG_DATA_GET(ZC_BUF_ID, ZC_SEG_ID);
INT         ZC_SEG_LEN_GET(ZC_BUF_ID, ZC_SEG_ID);
INT         ZCE_BUF_SEG_ERROR_CHECK(ZC_BUF_ID, ZC_SEG_ID);
