...
yaffs_Object *yaffsfs_FindObject(yaffs_Object *relativeDirectory, const char *path,int symDepth);
yaffs_Device *yaffsfs_FindDevice(const char *path, char **restOfPath);

char *yaffs_realpath( const char * path, char * resolved_path );

int yaffsfs_DoStat( yaffs_Object * obj, struct yaffs_stat * buf );
