...
const char * yaffsfs_c_version = "$Id: yaffsfs.c,v 1.11 2011/05/10 00:50:13 shenchen Exp $";
...
//
#ifndef WIN32
extern long long Get_system_clock_nanosecond(  );
#endif
...
static int yaffsfs_GetHandle( void )
{
    ...
    for( i = 0; i < YAFFSFS_N_HANDLES; i ++)
    {
        ...
        if(! h->inUse )
        {
            ...
            h->inUse = 1;
            h->tskOwner = NU_Current_Task_Pointer();
#ifndef WIN32
            h->openTs = Get_system_clock_nanosecond();
#endif
            return i;
        }
    }
...
int yaffs_rename( const char * oldPath, const char * newPath )
{
    ...
    if(! olddir || ! newdir || ! obj  || ( obj->inUse > 0 ) )
    {
        // bad file
        yaffsfs_SetError(- EBADF );
        renameAllowed = 0;
...
char * yaffs_realpath( const char * path, char * resolved_path )
{
#if 0
    yaffs_Object * obj = yaffsfs_FindObject( NULL, path, 0 );
    if(! obj )
        return NULL;
#endif
    char prePath[NAME_MAX], name[NAME_MAX], tmp[NAME_MAX],* str;
    yaffs_Object * objdir,* obj;
    int i, j;
    ...
    yaffsfs_Lock();
    objdir = yaffsfs_FindDirectory( NULL, prePath,& str, 0 );
    if(! objdir )
    {
        yaffsfs_Unlock();
        return NULL;
    ...
    if(! strlen( str ))
        ...
        obj = yaffsfs_FindObject( objdir, str, 0 );
    ...
    {//文件系统分区目录
        ...
        str = strchr(str,'/');
        *str = 0;
        strcpy(resolved_path,prePath);
        return resolved_path;
    }*/

    while( obj )
    {
        if( obj->objectId == YAFFS_OBJECTID_ROOT )
        {
            strcpy( name, prePath );
            str = & name[1];
            str = strchr( str, '/' );
        ...
        else  if( obj->objectId == YAFFS_OBJECTID_LOSTNFOUND )
            strcpy( name, YAFFS_LOSTNFOUND_NAME );
        else
            yaffs_GetObjectName( obj, name, NAME_MAX );
        strcpy( tmp, resolved_path );
        if( strlen( tmp ))
            sprintf( resolved_path, "%s/%s", name, tmp );
        else
